    ...
    IMarginVault
{
    ...
    function setDeps(address _riskConfig, address _oracleRouter, address _perpEngine)
        external
    ...
    function withdraw(address asset, uint256 amount, bool isolated, bytes32 marketId)
        external
        override
        nonReentrant
        whenNotPaused
    ...
    function reserve(address user, address asset, uint256 amount, bool isolated, bytes32 marketId)
        external
        override
        onlyRole(Constants.ENGINE)
    {
        if (isolated) {
    ...
    function release(address user, address asset, uint256 amount, bool isolated, bytes32 marketId)
        external
        override
        onlyRole(Constants.ENGINE)
    {
        ...
        uint256 z = collateralManager.assetValueInZUSD(asset, amount);
        if (reservedZ[user] >= z) reservedZ[user] -= z;
