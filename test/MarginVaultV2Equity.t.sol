...
contract MockCMEquity {
    ...
    function setTracked(address a) external { tracked = a; }
    function assetValueInZUSD(address, uint256 amount) external pure returns (uint256) { return amount; }
    function collateralValueInZUSD(address, uint256 amount) external pure returns (uint256) { return amount; }
...
contract MarginVaultV2EquityTest is Test {
    ...
    function testAccountEquitySumsCrossBalancesAndRespectsReserved() public {
        ...
        vm.prank(user); vault.deposit(address(token), 10 ether, false, 0);
